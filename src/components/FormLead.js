...
export class FormLead extends AppElement {
    ...
    static get observedAttributes() {
      ...
      attributeChangedCallback(name, oldValue, newValue) {
        if (oldValue==="awaiting" && newValue==="submited" ){
        ...
        if (this.state.phoneCodes===undefined){
            ...
            if (this.state.phoneCodes.length>0){
                this.state.phoneCodes.forEach(code=>{
                    let country = this.#countryCode.find(item => item.code === code)
                    values += `<option value="${country.dial_code}" ${this.state.code===country.code?'selected':''}>${this.#getFlagEmoji(country.code) + ' ' + country.dial_code }</option>`
                })
           }
        }
    ...
    handleEvent(event) {
        ...
        if (event.type === "change"&&event.target.id==='phone'){
            ...
            if (regex.test(event.target.value)){                
                ...
                leadForm.reset();
                leadForm.querySelectorAll('.help').forEach(el=>{
    ...
    addEvents(){
        if (this.state.context?.lang!=undefined){
    ...
    render(){
        ...
        ${this.state.context?.lang!=undefined?`
        <div class="columns is-centered">
            <div ${this.getClasses(["column"], this.state.form?.classList)}  ${this.setAnimation(this.state.form?.animation)}>
                ...
                ${this.state.name?.disabled!=true?`
                    ...
                    <div class="field${this.state.subject?.hidden===true?` is-hidden`:``}" ${this.setAnimation(this.state.subject.animation)}>
                        <label class="label">${this.state.subject?.label[this.state.context.lang]}</label>
                        <div class="control">
                        <input id="subject" class="input" ${this.state.subject?.hidden===true?`type="hidden" value="${this.state.subject?.value}"`:`type="text"`} ${this.state.subject?.placeholder!=undefined?`placeholder="${this.state.subject.placeholder[this.state.context.lang]}"`:``}  ${this.state.subject?.required===true?'required':''}>
                        </div>
                    </div>`:''}
